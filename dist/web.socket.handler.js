...
var WsHandler = /** @class */ (function () {
    function WsHandler(service, resource, elementUtil) {
        ...
        this.unsubscribeWebSocket = function (rsiWebSocket) {
            if (_this.subscriptions[rsiWebSocket.id]) {
                var subscriptions = _this.subscriptions[rsiWebSocket.id];
                for (var prop in subscriptions) {
                    if (subscriptions.hasOwnProperty(prop)) {
                        subscriptions[prop].unsubscribe();
                    }
                }
                delete _this.subscriptions[rsiWebSocket.id];
            }
        };
        /**
         ...
         */
        this.handleWebSocketMessages = function (msg, rsiWebSocket) {
            var eventObj = helpers_1.splitEvent(msg.event);
            // init if not yet initialized
            _this.subscriptions[rsiWebSocket.id] = _this.subscriptions[rsiWebSocket.id] || {};
            if (_this.subscriptions[rsiWebSocket.id][msg.event] && msg.type === "subscribe") {
                // logger.debug("Already subscribed: ", msg.event);
                return;
            }
            if (!eventObj.service || !eventObj.resource) {
                rsiWebSocket.sendError(msg.event, core_1.StatusCode.BAD_REQUEST, new Error("event url malformed"));
                return;
            }
            ...
            switch (msg.type) {
                case "subscribe":
                    if (eventObj.element) {
                        _this.handleElementSubscriptions(rsiWebSocket, msg, eventObj);
                    }
                    else {
                        _this.handleResourceSubscriptions(rsiWebSocket, msg, eventObj);
                    }
                    break;
                case "unsubscribe":
                    // logger.info("Unsubscription:", msg.event);
                    var subscription = _this.subscriptions[rsiWebSocket.id][msg.event];
                    if (subscription) {
                ...
                case "reauthorize":
                default:
                    _this.logger.error("Unsupported command on ws://:", msg.event);
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        if (element && subject_1) {
                            ...
                            this.subscriptions[rsiWebSocket.id][msg.event] = subscription$.subscribe(function (data) { return __awaiter(_this, void 0, void 0, function () {
